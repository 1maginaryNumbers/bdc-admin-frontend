...
const ActivityLog = () => {
  ...
  const fetchLogs = useCallback(async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:5000/api/activitylog?${params}`);
  ...
  const handleExport = async (format) => {
    try {
      ...
      const response = await axios.get(`http://localhost:5000/api/activitylog/export?${params}`, {
        responseType: 'blob'
      });

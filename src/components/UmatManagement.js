...
const UmatManagement = () => {
  ...
  const fetchUmat = useCallback(async () => {
    try {
      setLoading(true);
      const response = await axios.get(`http://localhost:5000/api/umat?page=${currentPage}&limit=20`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingUmat) {
        await axios.put(`http://localhost:5000/api/umat/${editingUmat._id}`, formData);
        toast.success('Umat updated successfully');
      } else {
        await axios.post('http://localhost:5000/api/umat', formData);
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this umat?')) {
      try {
        await axios.delete(`http://localhost:5000/api/umat/${id}`);

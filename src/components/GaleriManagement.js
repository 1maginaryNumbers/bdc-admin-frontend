...
import { FiEdit, FiTrash2, FiPlus, FiImage, FiUpload } from 'react-icons/fi';
...
const GaleriManagement = () => {
  ...
  const fetchGaleri = useCallback(async () => {
    try {
      setLoading(true);
      const response = await axios.get(`http://localhost:5000/api/galeri?page=${currentPage}&limit=20`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingGaleri) {
        await axios.put(`http://localhost:5000/api/galeri/${editingGaleri._id}`, formDataToSend, {
          headers: {
        ...
        await axios.post('http://localhost:5000/api/galeri', formDataToSend, {
          headers: {
            'Content-Type': 'multipart/form-data'
          }
        });
